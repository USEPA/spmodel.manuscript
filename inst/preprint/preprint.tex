...
model-based inference: point-referenced data and areal data (areal data
are sometimes called lattice data). Spatial data are point-referenced
...
\texttt{LatticeKrig} {[}9{]}, \texttt{R-inla} {[}10{]}, \texttt{rstan}
{[}11{]}, \texttt{spatial} {[}12{]}, \texttt{spBayes} {[}13{]}, and
\texttt{spNNGP} {[}14{]}. For areal data, they include \texttt{brms}
{[}15{]}, \texttt{CARBayes} {[}16{]} and \texttt{hglm} {[}17{]}. Unlike
these aforementioned packages, \texttt{spmodel} is designed to analyze
both point-referenced and areal data using a common framework and syntax
structure. \texttt{spmodel} also offers many features missing from the
...
We create visualizations using ggplot2 {[}18{]}, which we install from
...
of spatial data with \texttt{mapview} {[}19{]}. \texttt{mapview} has
...
tend to be more similar than observations far apart {[}20{]}. Failing to
...
One way to define \(\mathbf{W}\) is through queen contiguity {[}21{]}.
...
If \texttt{data} is an \texttt{sf} {[}23{]} object, spatial information
...
restricted maximum likelihood {[}24--26{]}. Maximum likelihood
estimation is also available. For point-referenced data, semivariogram
weighted least squares {[}27{]} and semivariogram composite likelihood
{[}28{]} are additional estimation methods. The estimation method is
...
{[}29{]} derive for spatial data. The AIC and AICc evaluate the fit of a
...
leave-one-out cross validation {[}31{]}. In leave-one-out cross
...
covariance matrix, \(\boldsymbol{\Sigma}\) {[}32{]}. The spatial hat
...
\(\boldsymbol{\Sigma}^{-1/2}\) yields the Pearson residuals {[}33{]}:
\begin{equation*}
...
the standardized residuals {[}33{]}: \begin{equation*}
...
{[}34{]}. For the non-spatial model, the Cook's distance of the \(i\)th
...
from the broom \textbf{\textsf{R}} package {[}35{]} provide convenient
...
uses row standardization {[}22{]} and assumes an independent error
...
calculating their profile likelihood confidence intervals {[}36{]}.
...
similar syntax as for random effects in the nlme {[}30{]} and lme4
{[}37{]} \textbf{\textsf{R}} packages. Random effects are specified via
...
\noindent \texttt{rand2} has the lowest AIC and AICc.
...
clustering {[}38{]} on the x-coordinates and y-coordinates to create 60
...
10. Lindgren F, Rue H. Bayesian spatial modelling with r-inla. Journal
of statistical software. 2015;63: 1--25.

\leavevmode\hypertarget{ref-rstan2023rstan}{}%
11. Stan Development Team. RStan: The R interface to Stan. 2023.
Available: \url{https://mc-stan.org/}

\leavevmode\hypertarget{ref-venables2002S}{}%
12. Venables WN, Ripley BD. Modern applied statistics with S. Fourth.
New York: Springer; 2002. Available:
\url{https://www.stats.ox.ac.uk/pub/MASS4/}

\leavevmode\hypertarget{ref-finley2007spbayes}{}%
13. Finley AO, Banerjee S, Carlin BP. spBayes: An R package for
...
14. Finley AO, Datta A, Banerjee S. spNNGP R package for nearest
...
15. Bürkner P-C. Brms: An r package for bayesian multilevel models using
stan. Journal of statistical software. 2017;80: 1--28.

\leavevmode\hypertarget{ref-lee2013carbayes}{}%
16. Lee D. CARBayes: An R package for Bayesian spatial modeling with
...
17. Ronnegard L, Shen X, Alam M. Hglm: A package for fitting
...
23. Pebesma E. Simple features for R: standardized support for spatial
...
24. Patterson D, Thompson R. Recovery of inter-block information when
block sizes are unequal. Biometrika. 1971;58: 545--554.

\leavevmode\hypertarget{ref-harville1977maximum}{}%
25. Harville DA. Maximum likelihood approaches to variance component
...
26. Wolfinger R, Tobias R, Sall J. Computing Gaussian likelihoods and
...
27. Cressie N. Fitting variogram models by weighted least squares.
Journal of the international Association for mathematical Geology.
1985;17: 563--586.

\leavevmode\hypertarget{ref-curriero1999composite}{}%
28. Curriero FC, Lele S. A composite likelihood approach to
...
38. MacQueen J, others. Some methods for classification and analysis of
